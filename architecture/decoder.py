...
class CrossAttentionBlock(torch.nn.Module):
    ...
    def __init__(
        ...
        self.norm_q = RMSNorm(config.hidden_size, device=device)
...
class TransformerBlock(torch.nn.Module):
    def __init__(
        ...
        if encoder_hidden_size is not None:
            self.cross_attn = CrossAttentionBlock(config, encoder_hidden_size, device)
...
class Transformer(torch.nn.Module):
    def __init__(
        ...
        self.block = torch.nn.ModuleList(
            [
                TransformerBlock(config, layer_idx, encoder_hidden_size, device)
